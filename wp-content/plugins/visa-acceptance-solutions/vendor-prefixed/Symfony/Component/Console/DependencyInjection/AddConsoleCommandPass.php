 ...
 */
namespace Pymt_Vas\Dependencies\Symfony\Component\Console\DependencyInjection;
...
use Pymt_Vas\Dependencies\Symfony\Component\Console\CommandLoader\ContainerCommandLoader;
    ...
    public function process(ContainerBuilder $container): void
    {
        ...
        foreach ($commandServices as $id => $tags) {
            ...
            if (Command::class !== (new \ReflectionMethod($class, 'getDefaultName'))->class) {
                pymt_vas_trigger_deprecation('symfony/console', '7.3', 'Overriding "Command::getDefaultName()" in "%s" is deprecated and will be removed in Symfony 8.0, use the #[AsCommand] attribute instead.', $class);
            ...
            if (!$description) {
                if (Command::class !== (new \ReflectionMethod($class, 'getDefaultDescription'))->class) {
                    pymt_vas_trigger_deprecation('symfony/console', '7.3', 'Overriding "Command::getDefaultDescription()" in "%s" is deprecated and will be removed in Symfony 8.0, use the #[AsCommand] attribute instead.', $class);
        ...
        $container->register('console.command_loader', ContainerCommandLoader::class)->setPublic(true)->addTag('container.no_preload')->setArguments([ServiceLocatorTagPass::register($container, $lazyCommandRefs), $lazyCommandMap]);
