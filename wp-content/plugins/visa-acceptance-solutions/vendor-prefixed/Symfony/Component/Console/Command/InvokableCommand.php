...
use Pymt_Vas\Dependencies\Symfony\Component\Console\Attribute\Argument;
use Pymt_Vas\Dependencies\Symfony\Component\Console\Attribute\Interact;
use Pymt_Vas\Dependencies\Symfony\Component\Console\Attribute\MapInput;
use Pymt_Vas\Dependencies\Symfony\Component\Console\Attribute\Option;
use Pymt_Vas\Dependencies\Symfony\Component\Console\Cursor;
...
use Pymt_Vas\Dependencies\Symfony\Component\Console\Interaction\Interaction;
    ...
    public function __invoke(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!\is_int($statusCode)) {
            if ($this->triggerDeprecations) {
                pymt_vas_trigger_deprecation('symfony/console', '7.3', \sprintf('Returning a non-integer value from the command "%s" is deprecated and will throw an exception in Symfony 8.0.', $this->command->getName()));
                return 0;
            }
    ...
    private function getParameters(\ReflectionFunction $function, InputInterface $input, OutputInterface $output): array
    {
        ...
        foreach ($function->getParameters() as $parameter) {
            ...
            if (!$type instanceof \ReflectionNamedType) {
                if ($this->triggerDeprecations) {
                    pymt_vas_trigger_deprecation('symfony/console', '7.3', \sprintf('Omitting the type declaration for the parameter "$%s" is deprecated and will throw an exception in Symfony 8.0.', $parameter->getName()));
                    continue;
                }
