...
class FileBag extends ParameterBag
{
    ...
    protected function fixPhpFilesArray(array $data): array
    {
        ...
        foreach ($data['name'] as $key => $name) {
            $files[$key] = $this->fixPhpFilesArray(['error' => $data['error'][$key], 'name' => $name, 'type' => $data['type'][$key], 'tmp_name' => $data['tmp_name'][$key], 'size' => $data['size'][$key]] + (isset($data['full_path'][$key]) ? ['full_path' => $data['full_path'][$key]] : []));
        }
        return $files;
    }
}