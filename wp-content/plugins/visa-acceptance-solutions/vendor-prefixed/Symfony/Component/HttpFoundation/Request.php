 ...
 */
namespace Pymt_Vas\Dependencies\Symfony\Component\HttpFoundation;

use Pymt_Vas\Dependencies\Symfony\Component\HttpFoundation\Exception\BadRequestException;
use Pymt_Vas\Dependencies\Symfony\Component\HttpFoundation\Exception\ConflictingHeadersException;
use Pymt_Vas\Dependencies\Symfony\Component\HttpFoundation\Exception\JsonException;
use Pymt_Vas\Dependencies\Symfony\Component\HttpFoundation\Exception\SessionNotFoundException;
use Pymt_Vas\Dependencies\Symfony\Component\HttpFoundation\Exception\SuspiciousOperationException;
...
class Request
{
    ...
    {
        pymt_vas_trigger_deprecation('symfony/http-foundation', '7.4', 'Request::get() is deprecated, use properties ->attributes, query or request directly instead.');
    ...
    public function getMethod(): string
    {
        ...
        if (\in_array($method, ['GET', 'HEAD', 'CONNECT', 'TRACE'], true)) {
            pymt_vas_trigger_deprecation('symfony/http-foundation', '7.4', 'HTTP method override is deprecated for methods GET, HEAD, CONNECT and TRACE; it will be ignored in Symfony 8.0.', $method);
        }
    ...
    {
        if (null === $format) {
            pymt_vas_trigger_deprecation('symfony/http-foundation', '7.4', 'Passing "null" as the first argument of "%s()" is deprecated. The argument will be non-nullable in Symfony 8.0.', __METHOD__);
            $format = '';
        }
        if (null === static::$formats) {
            static::initializeFormats();
