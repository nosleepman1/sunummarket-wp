 ...
 */
namespace Pymt_Vas\Dependencies\Symfony\Component\DependencyInjection\Loader;
...
use Pymt_Vas\Dependencies\Symfony\Component\DependencyInjection\Attribute\When;
use Pymt_Vas\Dependencies\Symfony\Component\DependencyInjection\Attribute\WhenNot;
...
use Pymt_Vas\Dependencies\Symfony\Component\DependencyInjection\Extension\ConfigurationExtensionInterface;
use Pymt_Vas\Dependencies\Symfony\Component\DependencyInjection\Extension\ExtensionInterface;
use Pymt_Vas\Dependencies\Symfony\Component\DependencyInjection\Loader\Configurator\App;
use Pymt_Vas\Dependencies\Symfony\Component\DependencyInjection\Loader\Configurator\AppReference;
...
class PhpFileLoader extends FileLoader
{
    ...
    public function __construct(ContainerBuilder $container, FileLocatorInterface $locator, ?string $env = null, ConfigBuilderGeneratorInterface|bool|null $generator = null, bool $prepend = false)
    {
    ...
    public function load(mixed $resource, ?string $type = null): mixed
    {
        ...
        try {
            ...
            if (\is_object($result) && \is_callable($result)) {
                ...
                ++$this->importing;
                try {
                    $loadContent->invoke($yamlLoader, ['imports' => ContainerConfigurator::processValue($result['imports'] ?? []), 'parameters' => ContainerConfigurator::processValue($result['parameters'] ?? []), 'services' => ContainerConfigurator::processValue($result['services'] ?? [], true)], $path);
                    ...
                    if ($this->env && isset($result[$when = 'when@' . $this->env])) {
                        ...
                        $loadContent->invoke($yamlLoader, ['imports' => ContainerConfigurator::processValue($result[$when]['imports'] ?? []), 'parameters' => ContainerConfigurator::processValue($result[$when]['parameters'] ?? []), 'services' => ContainerConfigurator::processValue($result[$when]['services'] ?? [], true)], $path);
        ...
        foreach ($r->getParameters() as $parameter) {
            ...
            switch ($type) {
                ...
                default:
                    ...
                    pymt_vas_trigger_deprecation('symfony/dependency-injection', '7.4', 'Using fluent builders for semantic configuration is deprecated, instantiate the "%s" class with the config array as argument and return it instead in "%s".', $type, $path);
