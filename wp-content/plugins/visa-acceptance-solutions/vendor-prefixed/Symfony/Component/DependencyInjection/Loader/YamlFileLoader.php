...
use Pymt_Vas\Dependencies\Symfony\Component\DependencyInjection\Alias;
use Pymt_Vas\Dependencies\Symfony\Component\DependencyInjection\Argument\AbstractArgument;
use Pymt_Vas\Dependencies\Symfony\Component\DependencyInjection\Argument\BoundArgument;
...
use Pymt_Vas\Dependencies\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument;
use Pymt_Vas\Dependencies\Symfony\Component\DependencyInjection\ChildDefinition;
    ...
    private function parseDefaults(array &$content, string $file): array
    {
        ...
        foreach (['tags', 'resource_tags'] as $type) {
            ...
            foreach ($tags as $tag) {
                ...
                $this->validateAttributes(\sprintf('Tag "%s", attribute "%s" in "_defaults" must be of a scalar-type in "%s".', $name, '%s', $file), $tag);
            }
        }
    ...
    private function parseDefinition(string $id, array|string|null $service, string $file, array $defaults, bool $return = false, bool $trackBindings = true): Definition|Alias|null
    {
        ...
        foreach (['tags', 'resource_tags'] as $type) {
            ...
            foreach ($tags as $tag) {
                ...
                $this->validateAttributes(\sprintf('A "%s" attribute must be of a scalar-type for service "%s", tag "%s", attribute "%s" in "%s".', $id, $name, '%s', $type, $file), $tag);
                match ($type) {
        ...
        if ($value instanceof TaggedValue) {
            ...
            if (\in_array($value->getTag(), ['tagged', 'tagged_iterator', 'tagged_locator'], true)) {
                if ('tagged' === $value->getTag()) {
                    pymt_vas_trigger_deprecation('symfony/dependency-injection', '7.2', 'Using "!tagged" is deprecated, use "!tagged_iterator" instead in "%s".', $file);
                }
