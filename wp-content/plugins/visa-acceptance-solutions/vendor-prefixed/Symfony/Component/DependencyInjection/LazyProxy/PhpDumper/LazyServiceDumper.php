 ...
 */
namespace Pymt_Vas\Dependencies\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper;
...
final class LazyServiceDumper implements DumperInterface
{
    ...
    public function getProxyClass(Definition $definition, bool $asGhostObject, ?\ReflectionClass &$class = null): string
    {
        ...
        return preg_replace('/^.*\\\\/', '', $definition->getClass()) . 'Proxy' . ucfirst(substr(hash('xxh128', $this->salt . '+' . $class->name . '+' . serialize($definition->getTag('proxy'))), -7));
    }
}