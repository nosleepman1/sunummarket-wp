...
use Pymt_Vas\Dependencies\Symfony\Component\HttpClient\Caching\Freshness;
use Pymt_Vas\Dependencies\Symfony\Component\HttpClient\Chunk\ErrorChunk;
use Pymt_Vas\Dependencies\Symfony\Component\HttpClient\Exception\ChunkCacheItemNotFoundException;
use Pymt_Vas\Dependencies\Symfony\Component\HttpClient\Response\AsyncContext;
use Pymt_Vas\Dependencies\Symfony\Component\HttpClient\Response\AsyncResponse;
use Pymt_Vas\Dependencies\Symfony\Component\HttpClient\Response\MockResponse;
use Pymt_Vas\Dependencies\Symfony\Component\HttpClient\Response\ResponseStream;
...
class CachingHttpClient implements HttpClientInterface, ResetInterface
{
     ...
     *                              to this maximum.
     */
    public function __construct(private HttpClientInterface $client, TagAwareCacheInterface|StoreInterface $cache, array $defaultOptions = [], private readonly bool $sharedCache = true, private readonly ?int $maxTtl = null)
    {
        if ($cache instanceof StoreInterface) {
            pymt_vas_trigger_deprecation('symfony/http-client', '7.4', 'Passing a "%s" as constructor\'s 2nd argument of "%s" is deprecated, "%s" expected.', StoreInterface::class, __CLASS__, TagAwareCacheInterface::class);
    ...
    public function request(string $method, string $url, array $options = []): ResponseInterface
    {
        ...
        $passthru = function (ChunkInterface $chunk, AsyncContext $context) use ($expiresAt, $fullUrlTag, $requestHash, $varyKey, $varyFields, &$metadataKey, $cachedData, $freshness, $url, $method, $options): \Generator {
            ...
            if ($chunk->isLast()) {
                $this->cache->get($chunkKey, static function (ItemInterface $item) use ($expiresAt, $fullUrlTag, $metadataKey, $chunk, $attemptTag): array {
                    ...
                    return ['content' => $chunk->getContent(), 'next_chunk' => null];
                }, \INF);
                ...
                $this->cache->get($metadataKey, static function (ItemInterface $item) use ($context, $headers, $maxAge, $expiresAt, $fullUrlTag, $metadataKey, $attemptTag, $firstChunkKey): array {
                    $item->tag([$fullUrlTag, $metadataKey, $attemptTag])->expiresAt($expiresAt);
                    return ['status_code' => $context->getStatusCode(), 'headers' => array_diff_key($headers, self::EXCLUDED_HEADERS), 'initial_age' => (int) ($headers['age'][0] ?? 0), 'stored_at' => time(), 'expires_at' => self::calculateExpiresAt($maxAge), 'next_chunk' => $firstChunkKey];
                }, \INF);
                yield $chunk;
                return;
            }
            ...
            $this->cache->get($chunkKey, static function (ItemInterface $item) use ($expiresAt, $fullUrlTag, $metadataKey, $attemptTag, $chunk, $nextChunkKey): array {
                $item->tag([$fullUrlTag, $metadataKey, $attemptTag])->expiresAt($expiresAt);
                return ['content' => $chunk->getContent(), 'next_chunk' => $nextChunkKey];
            }, \INF);
    ...
    private function determineMaxAge(array $headers, array $cacheControl): ?int
    {
        ...
        if (!isset($cacheControl['no-cache']) && !isset($cacheControl['no-store']) && isset($headers['last-modified'])) {
    ...
    private function isServerResponseCacheable(int $statusCode, array $requestHeaders, array $responseHeaders, array $cacheControl): bool
    {
        ...
        if ($this->sharedCache) {
            if (!isset($cacheControl['public']) && !isset($cacheControl['s-maxage']) && !isset($cacheControl['must-revalidate']) && isset($requestHeaders['authorization'])) {
                return false;
            }
    ...
    {
        return isset($headers['expires']) || $this->sharedCache && isset($cacheControl['s-maxage']) || isset($cacheControl['max-age']);
    }
    /**
    ...
    private function createResponseFromCache(array $cachedData, string $method, string $url, array $options, string $metadataKey, \DateTimeImmutable|false|null $newExpiresAt = false): MockResponse
    {
        ...
        return MockResponse::fromRequest($method, $url, $options, new MockResponse($body(), ['http_code' => $cachedData['status_code'], 'response_headers' => ['age' => $cachedData['initial_age'] + (time() - $cachedData['stored_at'])] + $cachedData['headers']]));
    }
