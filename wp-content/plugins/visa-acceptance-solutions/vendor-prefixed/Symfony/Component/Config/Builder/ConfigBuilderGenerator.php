 ...
 */
namespace Pymt_Vas\Dependencies\Symfony\Component\Config\Builder;

use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\ArrayNode;
use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\BaseNode;
use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\BooleanNode;
use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\Builder\ExprBuilder;
use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\ConfigurationInterface;
use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\EnumNode;
use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException;
use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\FloatNode;
use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\IntegerNode;
use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\NodeInterface;
use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\PrototypedArrayNode;
use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\ScalarNode;
use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\VariableNode;
use Pymt_Vas\Dependencies\Symfony\Component\Config\Loader\ParamConfigurator;
/**
...
public function NAME(): string
{
    ...
    private function handleArrayNode(ArrayNode $node, ClassBuilder $class, string $namespace): void
    {
        ...
        $property = $class->addProperty($node->getName(), $childClass->getFqcn() . ($acceptScalar ? '|scalar' : ''));
        $body = $acceptScalar ? '
...
{DEPRECATED_BODY
    ...
    return $this->PROPERTY;
        ...
        $class->addMethod($node->getName(), $body, ['DEPRECATED_BODY' => $class->isRoot ? "\n    \$this->_hasDeprecatedCalls = true;" : '', 'COMMENT' => $comment, 'PROPERTY' => $property->getName(), 'CLASS' => $childClass->getFqcn(), 'PARAM_TYPE' => $paramType]);
...
{DEPRECATED_BODY
    ...
    return $this;
        ...
        $class->addMethod($node->getName(), $body, ['DEPRECATED_BODY' => $class->isRoot ? "\n    \$this->_hasDeprecatedCalls = true;" : '', 'DEPRECATED_ANNOTATION' => $class->isRoot ? " @deprecated since Symfony 7.4\n *" : '', 'PROPERTY' => $property->getName(), 'COMMENT' => $comment, 'DEFAULT' => $node->hasDefaultValue() ? ' = ' . var_export($node->getDefaultValue(), true) : '']);
    }
...
{DEPRECATED_BODY
    ...
    return $this;
                ...
                $class->addMethod($node->getName(), $body, ['DEPRECATED_BODY' => $class->isRoot ? "\n    \$this->_hasDeprecatedCalls = true;" : '', 'DEPRECATED_ANNOTATION' => $class->isRoot ? " @deprecated since Symfony 7.4\n *" : '', 'PROPERTY' => $property->getName(), 'PROTOTYPE_TYPE' => implode('|', $prototypeParameterTypes), 'EXTRA_TYPE' => $nodeTypesWithoutArray ? '|' . implode('|', $nodeTypesWithoutArray) : '', 'PARAM_TYPE' => ['mixed'] !== $nodeParameterTypes ? 'ParamConfigurator|' . implode('|', $nodeParameterTypes) : 'mixed']);
            } else {
                $body = '
/**
 * @return $this
...
{DEPRECATED_BODY
    ...
    return $this;
                ...
                $class->addMethod($methodName, $body, ['DEPRECATED_BODY' => $class->isRoot ? "\n    \$this->_hasDeprecatedCalls = true;" : '', 'DEPRECATED_ANNOTATION' => $class->isRoot ? " @deprecated since Symfony 7.4\n *" : '', 'PROPERTY' => $property->getName(), 'TYPE' => ['mixed'] !== $prototypeParameterTypes ? 'ParamConfigurator|' . implode('|', $prototypeParameterTypes) : 'mixed', 'VAR' => '' === $key ? 'key' : $key, 'VALUE' => 'value' === $key ? 'data' : 'value']);
            }
            return;
        }
...
{DEPRECATED_BODY
    ...
    return $this->PROPERTY[$VAR];
            ...
            $class->addUse(InvalidConfigurationException::class);
            $class->addMethod($methodName, str_replace('$value', '$VAR', $body), ['DEPRECATED_BODY' => $class->isRoot ? "\n    \$this->_hasDeprecatedCalls = true;" : '', 'COMMENT' => $comment, 'PROPERTY' => $property->getName(), 'CLASS' => $childClass->getFqcn(), 'VAR' => '' === $key ? 'key' : $key, 'VALUE' => 'value' === $key ? 'data' : 'value', 'PARAM_TYPE' => $paramType]);
        }
...
{DEPRECATED_BODY
    ...
    return $this;
        ...
        $class->addMethod($node->getName(), $body, ['DEPRECATED_BODY' => $class->isRoot ? "\n    \$this->_hasDeprecatedCalls = true;" : '', 'DEPRECATED_ANNOTATION' => $class->isRoot ? " @deprecated since Symfony 7.4\n *" : '', 'PROPERTY' => $property->getName(), 'COMMENT' => $comment]);
    }
...
public function NAME(): array
{
    ...
    private function buildConstructor(ClassBuilder $class, NodeInterface $node): void
    {
        ...
        foreach ($class->getProperties() as $p) {
            ...
            if (null !== $p->getType()) {
                if ($p->isArray()) {
                    $code = $p->areScalarsAllowed() ? 'array_map(fn ($v) => \is_array($v) ? new ' . $p->getType() . '($v) : $v, $config[\'ORIG_NAME\'])' : 'array_map(fn ($v) => new ' . $p->getType() . '($v), $config[\'ORIG_NAME\'])';
                } else {
                    $code = $p->areScalarsAllowed() ? '\is_array($config[\'ORIG_NAME\']) ? new ' . $p->getType() . '($config[\'ORIG_NAME\']) : $config[\'ORIG_NAME\']' : 'new ' . $p->getType() . '($config[\'ORIG_NAME\'])';
                }
            }
            $body .= strtr('
