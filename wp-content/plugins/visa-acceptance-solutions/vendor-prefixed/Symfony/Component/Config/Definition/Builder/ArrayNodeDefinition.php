 ...
 */
namespace Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\Builder;

use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\ArrayNode;
use Pymt_Vas\Dependencies\Symfony\Component\Config\Definition\Exception\InvalidDefinitionException;
...
class ArrayNodeDefinition extends NodeDefinition implements ParentNodeDefinitionInterface
{
    ...
    {
        $disabledNode = $this->attribute('auto_enable', true)->addDefaultsIfNotSet()->treatFalseLike(['enabled' => false])->treatTrueLike(['enabled' => true])->treatNullLike(['enabled' => true])->beforeNormalization()->ifArray()->then(static function ($v) {
            $v['enabled'] ??= true;
            return $v;
    ...
    {
        $enabledNode = $this->attribute('auto_enable', true)->addDefaultsIfNotSet()->treatFalseLike(['enabled' => false])->treatTrueLike(['enabled' => true])->treatNullLike(['enabled' => true])->children()->booleanNode('enabled')->defaultTrue();
        $info = 1 <= \func_num_args() ? func_get_arg(0) : null;
        if ($info) {
