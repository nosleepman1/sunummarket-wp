 ...
 */
namespace Pymt_Vas\Dependencies\Symfony\Component\HttpKernel;
...
use Pymt_Vas\Dependencies\Symfony\Component\ErrorHandler\DebugClassLoader;
...
use Pymt_Vas\Dependencies\Symfony\Component\HttpKernel\Bundle\BundleInterface;
use Pymt_Vas\Dependencies\Symfony\Component\HttpKernel\CacheWarmer\WarmableInterface;
use Pymt_Vas\Dependencies\Symfony\Component\HttpKernel\Config\FileLocator;
use Pymt_Vas\Dependencies\Symfony\Component\HttpKernel\DependencyInjection\MergeExtensionConfigurationPass;
...
abstract class Kernel implements KernelInterface, RebootableInterface, TerminableInterface
{
    ...
    public function __construct(protected string $environment, protected bool $debug)
    {
        if (!$environment) {
    ...
    {
        pymt_vas_trigger_deprecation('symfony/http-kernel', '7.1', 'The "%s()" method is deprecated since Symfony 7.1 and will be removed in 8.0.', __METHOD__);
    ...
    protected function getContainerLoader(ContainerInterface $container): DelegatingLoader
    {
        ...
        $resolver = new LoaderResolver(array_merge(class_exists(XmlFileLoader::class) ? [new XmlFileLoader($container, $locator, $env)] : [], [new YamlFileLoader($container, $locator, $env), new IniFileLoader($container, $locator, $env), class_exists(XmlFileLoader::class, false) && class_exists(ConfigBuilderGenerator::class) ? new PhpFileLoader($container, $locator, $env, new ConfigBuilderGenerator($this->getBuildDir())) : new PhpFileLoader($container, $locator, $env), new GlobFileLoader($container, $locator, $env), new DirectoryLoader($container, $locator, $env), new ClosureLoader($container, $env)]));
    ...
    public function __serialize(): array
    {
        if (self::class === (new \ReflectionMethod($this, '__sleep'))->class || self::class !== (new \ReflectionMethod($this, '__serialize'))->class) {
            return ['environment' => $this->environment, 'debug' => $this->debug];
        }
        pymt_vas_trigger_deprecation('symfony/http-kernel', '7.4', 'Implementing "%s::__sleep()" is deprecated, use "__serialize()" instead.', get_debug_type($this));
        $data = [];
    ...
    public function __unserialize(array $data): void
    {
        if ($wakeup = self::class !== (new \ReflectionMethod($this, '__wakeup'))->class && self::class === (new \ReflectionMethod($this, '__unserialize'))->class) {
            pymt_vas_trigger_deprecation('symfony/http-kernel', '7.4', 'Implementing "%s::__wakeup()" is deprecated, use "__unserialize()" instead.', get_debug_type($this));
        }
        if (\in_array(array_keys($data), [['environment', 'debug'], ["\x00*\x00environment", "\x00*\x00debug"]], true)) {
            $environment = $data['environment'] ?? $data["\x00*\x00environment"];
            $debug = $data['debug'] ?? $data["\x00*\x00debug"];
        ...
        pymt_vas_trigger_deprecation('symfony/http-kernel', '7.4', 'Passing more than just key "environment" and "debug" to "%s::__unserialize()" is deprecated, populate properties in "%s::__unserialize()" instead.', self::class, get_debug_type($this));
    ...
    {
        pymt_vas_trigger_deprecation('symfony/http-kernel', '7.4', 'Calling "%s::__sleep()" is deprecated, use "__serialize()" instead.', get_debug_type($this));
    ...
    {
        pymt_vas_trigger_deprecation('symfony/http-kernel', '7.4', 'Calling "%s::__wakeup()" is deprecated, use "__unserialize()" instead.', get_debug_type($this));
        if (\is_object($this->environment) || \is_object($this->debug)) {
            throw new \BadMethodCallException('Cannot unserialize ' . __CLASS__);
        }
        $this->__construct($this->environment, $this->debug);
