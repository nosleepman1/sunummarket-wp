 ...
 */
namespace Pymt_Vas\Dependencies\Symfony\Component\String;

use Pymt_Vas\Dependencies\Symfony\Component\String\Exception\ExceptionInterface;
use Pymt_Vas\Dependencies\Symfony\Component\String\Exception\InvalidArgumentException;
/**
...
class UnicodeString extends AbstractUnicodeString
{
    ...
    public function __unserialize(array $data): void
    {
        if ($wakeup = self::class !== (new \ReflectionMethod($this, '__wakeup'))->class && self::class === (new \ReflectionMethod($this, '__unserialize'))->class) {
            pymt_vas_trigger_deprecation('symfony/string', '7.4', 'Implementing "%s::__wakeup()" is deprecated, use "__unserialize()" instead.', get_debug_type($this));
        }
        try {
            if (\in_array(array_keys($data), [['string'], ["\x00*\x00string"]], true)) {
            ...
            pymt_vas_trigger_deprecation('symfony/string', '7.4', 'Passing more than just key "string" to "%s::__unserialize()" is deprecated, populate properties in "%s::__unserialize()" instead.', self::class, get_debug_type($this));
    ...
    {
        pymt_vas_trigger_deprecation('symfony/string', '7.4', 'Calling "%s::__wakeup()" is deprecated, use "__unserialize()" instead.', get_debug_type($this));
        if (!\is_string($this->string)) {
            throw new \BadMethodCallException('Cannot unserialize ' . __CLASS__);
        }
        normalizer_is_normalized($this->string) ?: $this->string = normalizer_normalize($this->string);
