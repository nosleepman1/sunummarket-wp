...
namespace Pymt_Vas\Dependencies\CyberSource\Utilities\PGP\BatchUpload;
...
class MutualAuthUploadUtility
{
     ...
     * @throws ApiException
     */
    public static function uploadWithP12($encryptedPgpBytes, $endpointUrl, $fileName, $p12FilePath, $p12Password, $caCertPath = null, $verify_ssl = true, $logger = null)
    {
        return self::doMultipartUpload($encryptedPgpBytes, $endpointUrl, $fileName, ['type' => 'p12', 'cert' => $p12FilePath, 'password' => $p12Password], $caCertPath, $verify_ssl, $logger);
    }
    /**
     ...
     * @throws ApiException
     */
    public static function uploadWithKeyAndCert($encryptedPgpBytes, $endpointUrl, $fileName, $clientCertPath, $clientKeyPath, $caCertPath = null, $clientKeyPassword = null, $verify_ssl = true, $logger = null)
    {
        return self::doMultipartUpload($encryptedPgpBytes, $endpointUrl, $fileName, ['type' => 'pem', 'cert' => $clientCertPath, 'key' => $clientKeyPath, 'password' => $clientKeyPassword], $caCertPath, $verify_ssl, $logger);
    }
    /**
     * Shared logic for multipart upload with mTLS.
     */
    private static function doMultipartUpload($encryptedPgpBytes, $endpointUrl, $fileName, $tlsConfig, $caCertPath, $verify_ssl, $logger)
    {
        $boundary = uniqid();
        $formParams = [$fileName => $encryptedPgpBytes];
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, ["Content-Type: multipart/form-data; boundary=-------------{$boundary}", "v-c-correlation-id: {$correlationId}"]);
