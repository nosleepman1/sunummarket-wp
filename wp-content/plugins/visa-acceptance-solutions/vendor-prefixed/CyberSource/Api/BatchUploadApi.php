...
use Pymt_Vas\Dependencies\CyberSource\Utilities\PGP\BatchUpload\PgpEncryptionUtility;
use Pymt_Vas\Dependencies\CyberSource\Utilities\PGP\BatchUpload\MutualAuthUploadUtility;
use Pymt_Vas\Dependencies\CyberSource\Utilities\PGP\BatchUpload\BatchuploadUtility;
use Pymt_Vas\Dependencies\CyberSource\Logging\LogFactory;
...
class BatchUploadApi
{
     ...
     * ```php
     * $logConfig = new \Pymt_Vas\Dependencies\CyberSource\Logging\LogConfiguration();
     ...
     * $api = new \Pymt_Vas\Dependencies\CyberSource\Api\BatchUploadApi($logConfig);
     * ```
     *
     ...
     * @throws ApiException
     */
    public function uploadBatchWithP12($inputFilePath, $environmentHostname, $pgpEncryptionCertPath, $clientCertP12FilePath, $clientCertP12Password, $serverTrustCertPath = null, $verify_ssl = true)
    {
        if (self::$logger) {
        ...
        BatchuploadUtility::validateBatchApiP12Inputs($inputFilePath, $environmentHostname, $pgpEncryptionCertPath, $clientCertP12FilePath, $serverTrustCertPath);
        ...
        return MutualAuthUploadUtility::uploadWithP12($encryptedPgpBytes, $endpointUrl, $pgpFileName, $clientCertP12FilePath, $clientCertP12Password, $serverTrustCertPath, $verify_ssl, self::$logger);
    }
    /**
     ...
     * @throws ApiException
     */
    public function uploadBatchWithKeyAndCert($inputFilePath, $environmentHostname, $pgpEncryptionCertPath, $clientCertPath, $clientKeyPath, $serverTrustCertPath = null, $clientKeyPassword = null, $verify_ssl = true)
    {
        if (self::$logger) {
        ...
        return MutualAuthUploadUtility::uploadWithKeyAndCert($encryptedPgpBytes, $endpointUrl, $pgpFileName, $clientCertPath, $clientKeyPath, $serverTrustCertPath, $clientKeyPassword, $verify_ssl, self::$logger);
    }
    /**
    ...
    {
        $URL_PREFIX = 'https://';
        $baseUrl = stripos(trim($environmentHostname), $URL_PREFIX) === 0 ? trim($environmentHostname) : $URL_PREFIX . trim($environmentHostname);
