...
use Pymt_Vas\Dependencies\CyberSource\Logging\LogConfiguration as LogConfiguration;
...
class MerchantConfiguration
{
    ...
    public function __construct()
    {
        ...
        if (self::$logger === null) {
            self::$logger = (new LogFactory())->getLogger(\Pymt_Vas\Dependencies\CyberSource\Utilities\Helpers\ClassHelper::getClassName(get_class($this)), $this->logConfig);
        }
    }
    /**
    ...
    /**
     * Get the value of useMLEGlobally (legacy)
     * @deprecated Use getEnableRequestMLEForOptionalApisGlobally()
     * @return bool
     */
    ...
    /**
     * Set enableRequestMLEForOptionalApisGlobally (canonical).
     * Keeps legacy useMLEGlobally in sync every time.
     *
     * @param bool $enableRequestMLEForOptionalApisGlobally
     * @return $this
     */
    public function setEnableRequestMLEForOptionalApisGlobally($enableRequestMLEForOptionalApisGlobally)
    {
        $value = (bool) $enableRequestMLEForOptionalApisGlobally;
        $this->enableRequestMLEForOptionalApisGlobally = $value;
        $this->useMLEGlobally = $value;
        // keep legacy field synchronized
        return $this;
    }
    /**
     * Legacy setter for useMLEGlobally.
     * Delegates to setEnableRequestMLEForOptionalApisGlobally to ensure synchronization.
     *
    ...
    {
        return $this->setEnableRequestMLEForOptionalApisGlobally($useMLEGlobally);
    }
    /**
    ...
    public static function setMerchantCredentials($connectionDet)
    {
        ...
        // Prefer new field; fall back to legacy useMLEGlobally for backward compatibility.
        // If both are set, they must have the same value.
        if (isset($connectionDet->enableRequestMLEForOptionalApisGlobally) && isset($connectionDet->useMLEGlobally)) {
            if ((bool) $connectionDet->enableRequestMLEForOptionalApisGlobally !== (bool) $connectionDet->useMLEGlobally) {
                throw new InvalidArgumentException("useMLEGlobally and enableRequestMLEForOptionalApisGlobally must have the same value if both are set.");
            }
        }
        ...
        } elseif (isset($connectionDet->useMLEGlobally)) {
        ...
        if (isset($connectionDet->requestMleKeyAlias) && trim((string) $connectionDet->requestMleKeyAlias) !== '') {
            ...
            $config = $config->setMleKeyAlias($rawAlias);
    ...
    public function validateMerchantData()
    {
        ...
        self::$logger = (new LogFactory())->getLogger(\Pymt_Vas\Dependencies\CyberSource\Utilities\Helpers\ClassHelper::getClassName(get_class($this)), $this->getLogConfiguration());
        self::$logger->info(GlobalParameter::LOG_START_MSG);
        $logConfig = $this->getLogConfiguration();
        $configurationData = array(GlobalParameter::AUTHTYPE => $this->getAuthenticationType(), GlobalParameter::ENBLOGFIELD => $logConfig->getEnableLogging(), GlobalParameter::DEBUGLOGFILEPATH => $logConfig->getDebugLogFile(), GlobalParameter::ERRORLOGFILEPATH => $logConfig->getErrorLogFile(), GlobalParameter::RUNENVFIELD => $this->getRunEnvironment());
        $output = \Pymt_Vas\Dependencies\CyberSource\Utilities\Helpers\ListHelper::toString($configurationData);
    ...
    private function validateMLEConfiguration()
    {
        ...
        if ($responseMleConfigured) {
            ...
            $isP12File = false;
            if ($hasFilePath) {
                if (!file_exists($this->responseMlePrivateKeyFilePath) || !is_readable($this->responseMlePrivateKeyFilePath) || !is_file($this->responseMlePrivateKeyFilePath)) {
            ...
            } else if (!is_object($this->responseMlePrivateKey) || get_class($this->responseMlePrivateKey) !== 'OpenSSLAsymmetricKey') {
