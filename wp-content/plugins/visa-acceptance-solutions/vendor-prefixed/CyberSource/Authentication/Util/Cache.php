...
use Pymt_Vas\Dependencies\CyberSource\Authentication\Util\MLEException as MLEException;
class Cache
{
    ...
    {
        if (self::$logger === null) {
            self::$logger = (new LogFactory())->getLogger(\Pymt_Vas\Dependencies\CyberSource\Utilities\Helpers\ClassHelper::getClassName(get_class($this)), new \Pymt_Vas\Dependencies\CyberSource\Logging\LogConfiguration());
        }
    }
    public function updateCache($filePath, $merchantConfig)
    {
        ...
        self::$file_cache[$cacheKey] = ['private_key' => $privateKey, 'publicKey' => $publicKey, 'x509_certificate' => $x509Certificate, 'file_mod_time' => $fileModTime];
    }
    ...
    public function grabKeyFromPEM($filePath)
    {
        ...
        if (!isset(self::$file_cache[$cacheKey]) || self::$file_cache[$cacheKey]['file_mod_time'] !== $fileModTime) {
            ...
            self::$file_cache[$cacheKey] = ['private_key' => $privateKeyFromPEMFile, 'file_mod_time' => $fileModTime];
        }
    ...
    private function setupMLECache($merchantConfig, $cacheKey, $mleCertPath)
    {
        ...
        if (substr($lowercaseCacheKey, -strlen($respPrivKeyIdentifier)) === $respPrivKeyIdentifier) {
            ...
            try {
                ...
                self::$file_cache[$cacheKey] = ['response_mle_private_key' => $loaded, 'file_mod_time' => $fileModTime];
            } catch (\Exception $e) {
                if (self::$logger) {
                    self::$logger->error("Response MLE private key load failed: " . $e->getMessage());
                }
                self::$file_cache[$cacheKey] = ['response_mle_private_key' => null, 'file_mod_time' => $fileModTime];
            }
            return;
        }
        ...
        self::$file_cache[$cacheKey] = ['mle_cert' => $mleCert, 'file_mod_time' => $fileModTime];
    }
    /**
    ...
    public function getMLEKIdDataFromCache($merchantConfig)
    {
        ...
        if (!isset(self::$file_cache[$cacheKey])) {
            ...
            if ($cachedMLEKId === null || !isset($cachedMLEKId['file_mod_time']) || $cachedMLEKId['file_mod_time'] !== $currentModTime) {
    ...
    private function setUpMLEKIdCache($merchantConfig)
    {
        ...
        try {
            ...
            $merchantCert = Utility::getCertificateByAliasFromPKCS($filePath, $merchantConfig->getResponseMlePrivateKeyFilePassword(), $merchantConfig->getMerchantID());
            if ($merchantCert === null) {
                throw new MLEException("No certificate found for Response MLE Private Key file with merchant ID alias " . $merchantConfig->getMerchantID());
            }
            // Check if this is a CyberSource-generated P12 file
            $isCyberSourceP12 = Utility::isP12GeneratedByCyberSource($filePath, $merchantConfig->getResponseMlePrivateKeyFilePassword());
            $cachedMLEKId = ['kid' => null, 'file_mod_time' => file_exists($filePath) ? filemtime($filePath) : 0];
            if ($isCyberSourceP12) {
                try {
                    $mleKID = MLEUtility::extractSerialNumber($merchantCert, "Serial number not found in certificate subject field for Response MLE Private Key with merchant ID alias " . $merchantConfig->getMerchantID());
                    $cachedMLEKId['kid'] = $mleKID;
                } catch (\Exception $e) {
                    throw new MLEException("Failed to extract serial number from certificate for Response MLE: " . $e->getMessage(), 0, $e);
                }
            }
            self::$file_cache[$cacheKey] = $cachedMLEKId;
        } catch (\Exception $e) {
            self::$logger->error("Failed to load MLE KID from Response MLE Private Key file: {$filePath}. Error: " . $e->getMessage());
            // Store failure marker in cache
            if ($cacheKey !== null) {
                $failureMarker = ['kid' => null, 'file_mod_time' => 0];
