 ...
 *
 * @category Class
 * @package  Pymt_Vas\Dependencies\CyberSource\Authentication\Util\JWT
 * @author   CyberSource
 * @link     https://developer.cybersource.com/
 */
namespace Pymt_Vas\Dependencies\CyberSource\Authentication\Util\JWT;

use Exception;
use Pymt_Vas\Dependencies\Firebase\JWT\JWT;
use Pymt_Vas\Dependencies\Firebase\JWT\JWK;
...
class JWTUtility
{
    ...
    public static function verifyJwt(string $jwtToken, string|array $publicKey): void
    {
        ...
        if (!in_array($algorithm, self::SUPPORTED_ALGORITHMS)) {
            ...
            throw new JwtSignatureValidationException(sprintf('Unsupported JWT algorithm: %s. Supported algorithms: %s', $algorithm, $supportedAlgs));
        }
        ...
        } catch (\Pymt_Vas\Dependencies\Firebase\JWT\SignatureInvalidException $e) {
            throw new JwtSignatureValidationException('JWT signature verification failed', $e);
        } catch (\Pymt_Vas\Dependencies\Firebase\JWT\BeforeValidException $e) {
            throw new JwtSignatureValidationException('JWT not yet valid (nbf claim)', $e);
        } catch (\Pymt_Vas\Dependencies\Firebase\JWT\ExpiredException $e) {
