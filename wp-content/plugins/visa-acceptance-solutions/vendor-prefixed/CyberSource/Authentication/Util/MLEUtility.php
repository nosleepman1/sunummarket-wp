...
namespace Pymt_Vas\Dependencies\CyberSource\Authentication\Util;

use Pymt_Vas\Dependencies\CyberSource\Authentication\Util\GlobalParameter;
...
use Pymt_Vas\Dependencies\CyberSource\Authentication\Util\MLEException;
use Pymt_Vas\Dependencies\CyberSource\Authentication\Util\JWE\JWEUtility;
/*
...
class MLEUtility
{
    ...
    {
        $isMLEForAPI = false;
        if (isset($inboundMLEStatus) && strtolower($inboundMLEStatus) === 'optional' && $merchantConfig->getEnableRequestMLEForOptionalApisGlobally()) {
            $isMLEForAPI = true;
        }
        if (isset($inboundMLEStatus) && strtolower($inboundMLEStatus) === 'mandatory') {
    ...
    public static function encryptRequestPayload($merchantConfig, $requestBody)
    {
        ...
        if (self::$logger === null) {
            self::$logger = (new LogFactory())->getLogger(\Pymt_Vas\Dependencies\CyberSource\Utilities\Helpers\ClassHelper::getClassName(static::class), $merchantConfig->getLogConfiguration());
        }
        ...
        if ($merchantConfig->getLogConfiguration()->isMaskingEnabled()) {
            $printRequestBody = \Pymt_Vas\Dependencies\CyberSource\Utilities\Helpers\DataMasker::maskData($requestBody);
        } else {
    ...
    public static function decryptMleResponsePayload($merchantConfig, $mleResponseBody)
    {
        ...
        if ($merchantConfig->getLogConfiguration()->isMaskingEnabled()) {
            $maskedResponseBody = \Pymt_Vas\Dependencies\CyberSource\Utilities\Helpers\DataMasker::maskData($mleResponseBody);
        ...
        try {
            ...
            if ($merchantConfig->getLogConfiguration()->isMaskingEnabled()) {
                $maskedDecrypted = \Pymt_Vas\Dependencies\CyberSource\Utilities\Helpers\DataMasker::maskData($decrypted);
    ...
    {
        try {
            $serialNumber = self::extractSerialNumber($cert, "Serial number not found in certificate subject field for Request MLE encryption");
            $publicKey = openssl_pkey_get_details(openssl_pkey_get_public($cert))['key'];
            $jwk = JWKFactory::createFromKey($publicKey, null, ['kid' => $serialNumber]);
            $header = ['alg' => 'RSA-OAEP-256', 'enc' => 'A256GCM', 'cty' => 'JWT', 'kid' => $serialNumber, 'iat' => time()];
            $algorithmManager = new AlgorithmManager([new RSAOAEP(), new RSAOAEP256(), new A256GCM()]);
            $compressionManager = new CompressionMethodManager([new Deflate()]);
            $jweBuilder = new JWEBuilder($algorithmManager, $algorithmManager, $compressionManager);
            $jwe = $jweBuilder->create()->withPayload($requestBody)->withSharedProtectedHeader($header)->addRecipient($jwk)->build();
     ...
     * 
     * @param \Pymt_Vas\Dependencies\CyberSource\Authentication\Core\MerchantConfiguration $merchantConfig The merchant configuration
    ...
    public static function validateAndAutoExtractResponseMleKid($merchantConfig)
    {
        ...
        if ($cybsKid === null && $configuredKID === null) {
            throw new MLEException("responseMleKID is required when response MLE is enabled. " . "Could not auto-extract from certificate and no manual configuration provided. " . "Please provide responseMleKID explicitly in your configuration.");
