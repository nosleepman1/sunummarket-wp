 ...
 */
namespace Pymt_Vas\Dependencies\CyberSource;
 
use Pymt_Vas\Dependencies\CyberSource\Authentication\Core\Authentication as Authentication;
...
class ApiClient
{
     ...
     */
    public function __construct(\Pymt_Vas\Dependencies\CyberSource\Configuration $config = null, \Pymt_Vas\Dependencies\CyberSource\Authentication\Core\MerchantConfiguration $merchantConfig = null)
    {
        if ($config === null) {
        ...
        if (self::$logger === null) {
            self::$logger = (new LogFactory())->getLogger(\Pymt_Vas\Dependencies\CyberSource\Utilities\Helpers\ClassHelper::getClassName(get_class($this)), $merchantConfig->getLogConfiguration());
        }
    }
    /**
     * Get Client ID
    ...
    {
        $versionInfo = "cybs-rest-sdk-php-prefixed";
        if (class_exists('\Composer\InstalledVersions')) {
            try {
                ...
                // Package not found in InstalledVersions (vendor-prefixed scenario)
                // Fall back to checking the vendor-prefixed composer installed.json
            }
        }
       
        // Try to get version from vendor-prefixed composer installed.json
        $filePath = __DIR__ . "/../composer/installed.json";
     ...
     *
     * @throws \Pymt_Vas\Dependencies\CyberSource\ApiException on a non 2xx response
     * @return mixed
     */
    public function callApi($resourcePath, $method, $queryParams, $postData, $headerParams, $responseType = null, $endpointPath = null, $isResponseMLEForAPI = false)
    {
        ...
        if ($postData and in_array('Content-Type: application/x-www-form-urlencoded', $headers, true)) {
            ...
            // json model
            $postData = json_encode(\Pymt_Vas\Dependencies\CyberSource\ObjectSerializer::sanitizeForSerialization($postData));
        }
        ...
        self::$logger->debug("Request Headers :\n" . \Pymt_Vas\Dependencies\CyberSource\Utilities\Helpers\DataMasker::maskAuthenticationData(\Pymt_Vas\Dependencies\CyberSource\Utilities\Helpers\ListHelper::toString($reqHeaders)));
        $curl = curl_init();
        ...
        if ($this->config->getTlsCipherList()) {
            if (defined('CURL_SSLVERSION_TLSv1_3') && defined('CURLOPT_TLS13_CIPHERS') && $this->config->getTlsVersion() === CURL_SSLVERSION_TLSv1_3) {
        ...
        if ($this->merchantConfig->getLogConfiguration()->isMaskingEnabled()) {
            $printPostData = \Pymt_Vas\Dependencies\CyberSource\Utilities\Helpers\DataMasker::maskData($postData);
        } else {
        ...
        if (!isset($this->downloadFilePath) && trim($this->downloadFilePath ?? '') == '') {
            ...
            if ($this->merchantConfig->getLogConfiguration()->isMaskingEnabled()) {
                $printHttpBody = \Pymt_Vas\Dependencies\CyberSource\Utilities\Helpers\DataMasker::maskData($http_body);
            } else {
            ...
            if ($response_info['http_code'] === 0) {
                ...
                self::$logger->error("ApiException : [" . $response_info['http_code'] . "] Error connecting to the API ({$url})");
                self::$logger->close();
                throw new ApiException("[" . $response_info['http_code'] . "] Error connecting to the API ({$url})", $response_info['http_code'], $stream_headers, null);
            }
        }
    }
    ...
    public function callAuthenticationHeader($method, $postData, $resourcePath, $isResponseMLEForAPI)
    {
        ...
        if ($merchantConfig->getAuthenticationType() == GlobalParameter::HTTP_SIGNATURE) {
            ...
            $headers = array($vcMerchant, $getToken, $host, 'Date:' . $date);
        } else if ($merchantConfig->getAuthenticationType() == GlobalParameter::JWT) {
            $headers = array('Authorization:' . $getToken);
