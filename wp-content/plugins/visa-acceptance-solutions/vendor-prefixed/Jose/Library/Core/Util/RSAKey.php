...
use Pymt_Vas\Dependencies\SpomkyLabs\Pki\CryptoEncoding\PEM;
use Pymt_Vas\Dependencies\SpomkyLabs\Pki\CryptoTypes\AlgorithmIdentifier\Asymmetric\RSAEncryptionAlgorithmIdentifier;
use Pymt_Vas\Dependencies\SpomkyLabs\Pki\CryptoTypes\Asymmetric\RSA\RSAPrivateKey;
use Pymt_Vas\Dependencies\SpomkyLabs\Pki\CryptoTypes\Asymmetric\RSA\RSAPublicKey;
...
final class RSAKey
{
    ...
    public function toPEM(): string
    {
        if (array_key_exists('d', $this->values)) {
            $this->sequence = Sequence::create(Integer::create(0), RSAEncryptionAlgorithmIdentifier::create()->toASN1(), OctetString::create(RSAPrivateKey::create($this->fromBase64ToInteger($this->values['n']), $this->fromBase64ToInteger($this->values['e']), $this->fromBase64ToInteger($this->values['d']), isset($this->values['p']) ? $this->fromBase64ToInteger($this->values['p']) : '0', isset($this->values['q']) ? $this->fromBase64ToInteger($this->values['q']) : '0', isset($this->values['dp']) ? $this->fromBase64ToInteger($this->values['dp']) : '0', isset($this->values['dq']) ? $this->fromBase64ToInteger($this->values['dq']) : '0', isset($this->values['qi']) ? $this->fromBase64ToInteger($this->values['qi']) : '0')->toDER()));
            return PEM::create(PEM::TYPE_PRIVATE_KEY, $this->sequence->toDER())->string();
        }
        $this->sequence = Sequence::create(RSAEncryptionAlgorithmIdentifier::create()->toASN1(), BitString::create(RSAPublicKey::create($this->fromBase64ToInteger($this->values['n']), $this->fromBase64ToInteger($this->values['e']))->toDER()));
        return PEM::create(PEM::TYPE_PUBLIC_KEY, $this->sequence->toDER())->string();
    }
    /**
    ...
    public static function exponentiate(self $key, BigInteger $c): BigInteger
    {
        ...
        if ($key->isPublic() || $key->getCoefficient() === null || count($key->getPrimes()) === 0 || count($key->getExponents()) === 0) {
    ...
    private function fromBase64ToInteger(string $value): string
    {
        ...
        return \Pymt_Vas\Dependencies\Brick\Math\BigInteger::fromBase(current($unpacked), 16)->toBase(10);
    }
}