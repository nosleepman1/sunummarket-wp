...
use Pymt_Vas\Dependencies\Jose\Component\Core\Util\KeyChecker;
use Pymt_Vas\Dependencies\Jose\Component\Encryption\Algorithm\ContentEncryptionAlgorithm;
use Pymt_Vas\Dependencies\Jose\Component\Encryption\Algorithm\KeyEncryption\DirectEncryption;
use Pymt_Vas\Dependencies\Jose\Component\Encryption\Algorithm\KeyEncryption\KeyAgreement;
use Pymt_Vas\Dependencies\Jose\Component\Encryption\Algorithm\KeyEncryption\KeyAgreementWithKeyWrapping;
use Pymt_Vas\Dependencies\Jose\Component\Encryption\Algorithm\KeyEncryption\KeyEncryption;
use Pymt_Vas\Dependencies\Jose\Component\Encryption\Algorithm\KeyEncryption\KeyWrapping;
use Pymt_Vas\Dependencies\Jose\Component\Encryption\Algorithm\KeyEncryptionAlgorithm;
use Pymt_Vas\Dependencies\Jose\Component\Encryption\Compression\CompressionMethod;
...
class JWEBuilder
{
    ...
    public function __construct(AlgorithmManager $algorithmManager, null|AlgorithmManager $contentEncryptionAlgorithmManager = null, private readonly null|CompressionMethodManager $compressionManager = null)
    {
        if ($compressionManager !== null) {
            pymt_vas_trigger_deprecation('web-token/jwt-library', '3.3.0', 'The parameter "$compressionManager" is deprecated and will be removed in 4.0.0. Compression is not recommended for JWE. Please set "null" instead.');
        }
        if ($contentEncryptionAlgorithmManager !== null) {
            pymt_vas_trigger_deprecation('web-token/jwt-library', '3.3.0', 'The parameter "$contentEncryptionAlgorithmManager" is deprecated and will be removed in 4.0.0. Please set all algorithms in the first argument and set "null" instead.');
    ...
    public function addRecipient(JWK $recipientKey, array $recipientHeader = []): self
    {
        ...
        } else if (!$clone->areKeyManagementModesCompatible($clone->keyManagementMode, $keyEncryptionAlgorithm->getKeyManagementMode())) {
        ...
        $clone->recipients[] = ['key' => $recipientKey, 'header' => $recipientHeader, 'key_encryption_algorithm' => $keyEncryptionAlgorithm];
        return $clone;
    }
    ...
    public function build(): JWE
    {
        ...
        $encodedSharedProtectedHeader = count($sharedProtectedHeader) === 0 ? '' : Base64UrlSafe::encodeUnpadded(JsonConverter::encode($sharedProtectedHeader));
    ...
    private function processRecipient(array $recipient, string $cek, array &$additionalHeader): Recipient
    {
        ...
        $encryptedContentEncryptionKey = $this->getEncryptedKey($completeHeader, $cek, $keyEncryptionAlgorithm, $additionalHeader, $recipient['key'], $recipient['sender_key'] ?? $this->senderKey ?? null);
