...
namespace Pymt_Vas\Dependencies\Jose\Component\Encryption\Algorithm\KeyEncryption\Util;

use InvalidArgumentException;
use Pymt_Vas\Dependencies\Jose\Component\Core\Util\BigInteger;
use Pymt_Vas\Dependencies\Jose\Component\Core\Util\Hash;
...
final class RSACrypt
{
    ...
    public static function decryptWithRSAOAEP(RSAKey $key, string $ciphertext, string $hash_algorithm): string
    {
        ...
        $splitCiphertext[count($splitCiphertext) - 1] = str_pad($splitCiphertext[count($splitCiphertext) - 1], $key->getModulusLength(), chr(0), STR_PAD_LEFT);
        $plaintext = '';
