...
use Pymt_Vas\Dependencies\Jose\Component\Core\Util\Ecc\EcDH;
use Pymt_Vas\Dependencies\Jose\Component\Core\Util\Ecc\NistCurve;
use Pymt_Vas\Dependencies\Jose\Component\Core\Util\Ecc\PrivateKey;
use Pymt_Vas\Dependencies\Jose\Component\Core\Util\ECKey;
use Pymt_Vas\Dependencies\Jose\Component\Encryption\Algorithm\KeyEncryption\Util\ConcatKDF;
...
abstract class AbstractECDH implements KeyAgreement
{
    ...
    public function getAgreementKey(int $encryptionKeyLength, string $algorithm, JWK $recipientKey, ?JWK $senderKey, array $complete_header = [], array &$additional_header_values = []): string
    {
        if ($recipientKey->has('d')) {
            ...
            [$public_key, $private_key] = $this->getKeysFromPublicKey($recipientKey, $senderKey, $additional_header_values);
        }
     ...
     * @return JWK[]
     */
    protected function getKeysFromPublicKey(JWK $recipient_key, ?JWK $senderKey, array &$additional_header_values): array
    {
    ...
    private function createOKPKey(string $curve): JWK
    {
        ...
        return new JWK(['kty' => 'OKP', 'crv' => $curve, 'x' => Base64UrlSafe::encodeUnpadded($x), 'd' => Base64UrlSafe::encodeUnpadded($d)]);
    }
