...
namespace Pymt_Vas\Dependencies\Jose\Component\Encryption\Serializer;
...
use Pymt_Vas\Dependencies\Jose\Component\Encryption\Recipient;
...
final class JSONFlattenedSerializer implements JWESerializer
{
    ...
    public function serialize(JWE $jwe, ?int $recipientIndex = null): string
    {
        ...
        $data = ['ciphertext' => Base64UrlSafe::encodeUnpadded($jwe->getCiphertext() ?? ''), 'iv' => Base64UrlSafe::encodeUnpadded($jwe->getIV() ?? ''), 'tag' => Base64UrlSafe::encodeUnpadded($jwe->getTag() ?? '')];
        if ($jwe->getAAD() !== null) {
    ...
    public function unserialize(string $input): JWE
    {
        ...
        $encryptedKey = array_key_exists('encrypted_key', $data) ? Base64UrlSafe::decodeNoPadding($data['encrypted_key']) : null;
        $header = array_key_exists('header', $data) ? $data['header'] : [];
        return new JWE($ciphertext, $iv, $tag, $aad, $sharedHeader, $sharedProtectedHeader, $encodedSharedProtectedHeader, [new Recipient($header, $encryptedKey)]);
    }
    ...
    private function processHeaders(array $data): array
    {
        ...
        $sharedProtectedHeader = $encodedSharedProtectedHeader ? JsonConverter::decode(Base64UrlSafe::decodeNoPadding($encodedSharedProtectedHeader)) : [];
