...
use Pymt_Vas\Dependencies\Jose\Component\Signature\Algorithm\MacAlgorithm;
use Pymt_Vas\Dependencies\Jose\Component\Signature\Algorithm\SignatureAlgorithm;
use LogicException;
...
class JWSBuilder
{
    ...
    public function __construct(private readonly AlgorithmManager $signatureAlgorithmManager)
    {
    }
    /**
    ...
    public function addSignature(JWK $signatureKey, array $protectedHeader, array $header = []): self
    {
        ...
        $clone = clone $this;
        $clone->signatures[] = ['signature_algorithm' => $algorithm, 'signature_key' => $signatureKey, 'protected_header' => $protectedHeader, 'header' => $header];
        return $clone;
    }
    /**
    ...
    public function build(): JWS
    {
        ...
        $encodedPayload = $this->isPayloadEncoded === false ? $this->payload : Base64UrlSafe::encodeUnpadded($this->payload);
        ...
        foreach ($this->signatures as $signature) {
            ...
            $encodedProtectedHeader = count($protectedHeader) === 0 ? null : Base64UrlSafe::encodeUnpadded(JsonConverter::encode($protectedHeader));
