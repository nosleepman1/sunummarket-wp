...
use function count;
/**
 * @see \Pymt_Vas\Dependencies\Jose\Tests\Component\Signature\JWSTest
 */
class JWS implements JWT
{
    ...
    public function __construct(private readonly ?string $payload, private readonly ?string $encodedPayload = null, private readonly bool $isPayloadDetached = false)
    {
    }
    ...
    public function split(): array
    {
        ...
        foreach ($this->signatures as $signature) {
            ...
            $jws = $jws->addSignature($signature->getSignature(), $signature->getProtectedHeader(), $signature->getEncodedProtectedHeader(), $signature->getHeader());
            $result[] = $jws;
        }
        return $result;
    }
