...
class JWSVerifier
{
    ...
    private function getInputToVerify(JWS $jws, Signature $signature, ?string $detachedPayload): string
    {
        ...
        $isPayloadBase64Encoded = !$signature->hasProtectedHeaderParameter('b64') || $signature->getProtectedHeaderParameter('b64') === true;
        ...
        $callable = $isPayloadBase64Encoded === true ? static fn(?string $p): string => Base64UrlSafe::encodeUnpadded($p ?? '') : static fn(?string $p): string => $p ?? '';
