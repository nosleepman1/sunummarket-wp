...
final class JSONGeneralSerializer extends Serializer
{
    ...
    public function serialize(JWS $jws, ?int $signatureIndex = null): string
    {
        ...
        foreach ($jws->getSignatures() as $signature) {
            ...
            $values = ['protected' => $signature->getEncodedProtectedHeader(), 'header' => $signature->getHeader()];
    ...
    public function unserialize(string $input): JWS
    {
        ...
        foreach ($data['signatures'] as $signature) {
            ...
            $signatures[] = ['signature' => Base64UrlSafe::decodeNoPadding($signature['signature']), 'protected' => $protectedHeader, 'encoded_protected' => $encodedProtectedHeader, 'header' => $header];
        ...
        foreach ($signatures as $signature) {
            $jws = $jws->addSignature($signature['signature'], $signature['protected'], $signature['encoded_protected'], $signature['header']);
        }
        return $jws;
    }
    /**
    ...
    private function processHeaders(array $signature): array
    {
        ...
        $protectedHeader = $encodedProtectedHeader === null ? [] : JsonConverter::decode(Base64UrlSafe::decodeNoPadding($encodedProtectedHeader));
