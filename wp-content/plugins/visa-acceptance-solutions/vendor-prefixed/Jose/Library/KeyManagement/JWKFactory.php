...
use Pymt_Vas\Dependencies\Jose\Component\KeyManagement\KeyConverter\KeyConverter;
...
/**
 * @see \Pymt_Vas\Dependencies\Jose\Tests\Component\KeyManagement\JWKFactoryTest
 */
class JWKFactory
{
    /**
    ...
    public static function createOKPKey(string $curve, array $values = []): JWK
    {
        ...
        $values = [...$values, 'kty' => 'OKP', 'crv' => $curve, 'd' => Base64UrlSafe::encodeUnpadded($d), 'x' => Base64UrlSafe::encodeUnpadded($x)];
        return new JWK($values);
    }
    /**
    ...
    {
        $values = array_merge($additional_values, ['kty' => 'oct', 'k' => Base64UrlSafe::encodeUnpadded($secret)]);
        return new JWK($values);
    }
    /**
