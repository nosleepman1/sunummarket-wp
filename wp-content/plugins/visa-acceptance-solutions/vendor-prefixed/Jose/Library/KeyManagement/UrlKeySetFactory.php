...
use Pymt_Vas\Dependencies\Psr\Cache\CacheItemPoolInterface;
use Pymt_Vas\Dependencies\Psr\Http\Client\ClientInterface;
use Pymt_Vas\Dependencies\Psr\Http\Message\RequestFactoryInterface;
use RuntimeException;
use Symfony\Component\Cache\Adapter\NullAdapter;
use Pymt_Vas\Dependencies\Symfony\Contracts\HttpClient\HttpClientInterface;
use function assert;
/**
 * @see \Pymt_Vas\Dependencies\Jose\Tests\Component\KeyManagement\UrlKeySetFactoryTest
 */
abstract class UrlKeySetFactory
{
    private CacheItemPoolInterface $cacheItemPool;
    private int $expiresAfter = 3600;
    public function __construct(private readonly ClientInterface|HttpClientInterface $client, private readonly null|RequestFactoryInterface $requestFactory = null)
    {
        if ($this->client instanceof ClientInterface) {
            pymt_vas_trigger_deprecation('web-token/jwt-library', '3.3', 'Using "%s" with an instance of "%s" is deprecated, use "%s" instead.', self::class, ClientInterface::class, HttpClientInterface::class);
        }
        if (!$this->client instanceof HttpClientInterface && $this->requestFactory === null) {
            throw new RuntimeException(sprintf('The request factory must be provided when using an instance of "%s" as client.', ClientInterface::class));
        }
    ...
    protected function getContent(string $url, array $header = []): string
    {
        ...
        $content = $this->client instanceof HttpClientInterface ? $this->sendSymfonyRequest($url, $header) : $this->sendPsrRequest($url, $header);
        $item = $this->cacheItemPool->getItem($cacheKey);
