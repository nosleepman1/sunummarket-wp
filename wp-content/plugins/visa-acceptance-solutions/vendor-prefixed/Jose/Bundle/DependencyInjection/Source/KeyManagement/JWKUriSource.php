...
use Pymt_Vas\Dependencies\Jose\Bundle\JoseFramework\Controller\JWKSetController;
...
class JWKUriSource implements Source
{
    ...
    {
        $node->children()->arrayNode('jwk_uris')->treatFalseLike([])->treatNullLike([])->useAttributeAsKey('name')->arrayPrototype()->children()->scalarNode('id')->info('The service ID of the Key Set to share.')->isRequired()->end()->scalarNode('path')->info('To share the JWKSet, then set a valid path (e.g. "/jwkset.json").')->isRequired()->end()->arrayNode('tags')->info('A list of tags to be associated to the service.')->useAttributeAsKey('name')->treatNullLike([])->treatFalseLike([])->variablePrototype()->end()->end()->booleanNode('is_public')->info('If true, the service will be public, else private.')->defaultTrue()->end()->end()->end()->end()->end();
    }
