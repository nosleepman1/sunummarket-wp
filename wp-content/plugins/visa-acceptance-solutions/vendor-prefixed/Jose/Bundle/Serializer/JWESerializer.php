...
namespace Pymt_Vas\Dependencies\Jose\Bundle\JoseFramework\Serializer;
...
final class JWESerializer implements DenormalizerInterface
{
    ...
    public function __construct(JWESerializerManagerFactory $serializerManagerFactory, ?JWESerializerManager $serializerManager = null)
    {
    ...
    {
        return [JWE::class => class_exists(JWESerializerManager::class) && $this->formatSupported($format)];
    }
    public function supportsDenormalization(mixed $data, string $type, ?string $format = null, array $context = []): bool
    {
        return $type === JWE::class && class_exists(JWESerializerManager::class) && $this->formatSupported($format);
    }
    ...
    {
        return $format !== null && in_array(mb_strtolower($format), $this->serializerManager->names(), true);
    }
}