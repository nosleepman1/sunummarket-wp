...
final class JWSSerializer implements DenormalizerInterface
{
    ...
    public function __construct(JWSSerializerManagerFactory $serializerManagerFactory, ?JWSSerializerManager $serializerManager = null)
    {
    ...
    {
        return [JWS::class => class_exists(JWSSerializerManager::class) && $this->formatSupported($format)];
    }
    public function supportsDenormalization(mixed $data, string $type, ?string $format = null, array $context = []): bool
    {
        return $type === JWS::class && class_exists(JWSSerializerManager::class) && $this->formatSupported($format);
    }
    ...
    {
        return $format !== null && in_array(mb_strtolower($format), $this->serializerManager->list(), true);
    }
}