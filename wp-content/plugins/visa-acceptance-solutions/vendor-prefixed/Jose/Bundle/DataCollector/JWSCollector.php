...
class JWSCollector implements Collector, EventSubscriberInterface
{
    ...
    public function __construct(private readonly ?JWSSerializerManagerFactory $jwsSerializerManagerFactory = null)
    {
    }
    /**
    ...
    {
        return [JWSVerificationSuccessEvent::class => ['catchJwsVerificationSuccess'], JWSVerificationFailureEvent::class => ['catchJwsVerificationFailure'], JWSBuiltSuccessEvent::class => ['catchJwsBuiltSuccess'], JWSBuiltFailureEvent::class => ['catchJwsBuiltFailure']];
    }
    ...
    private function collectSupportedJWSBuilders(array &$data): void
    {
        ...
        foreach ($this->jwsBuilders as $id => $jwsBuilder) {
            $data['jws']['jws_builders'][$id] = ['signature_algorithms' => $jwsBuilder->getSignatureAlgorithmManager()->list()];
        }
    }
    /**
     * @param array<string, array<string, mixed>> $data
    ...
    private function collectSupportedJWSVerifiers(array &$data): void
    {
        ...
        foreach ($this->jwsVerifiers as $id => $jwsVerifier) {
            $data['jws']['jws_verifiers'][$id] = ['signature_algorithms' => $jwsVerifier->getSignatureAlgorithmManager()->list()];
        }
    }
    /**
     * @param array<string, array<string, mixed>> $data
    ...
    private function collectSupportedJWSLoaders(array &$data): void
    {
        ...
        foreach ($this->jwsLoaders as $id => $jwsLoader) {
            $data['jws']['jws_loaders'][$id] = ['serializers' => $jwsLoader->getSerializerManager()->list(), 'signature_algorithms' => $jwsLoader->getJwsVerifier()->getSignatureAlgorithmManager()->list()];
        }
    }
    /**
     * @param array<string, array<string, mixed>> $data
    ...
    {
        $data['jws']['events'] = ['verification_success' => $this->jwsVerificationSuccesses, 'verification_failure' => $this->jwsVerificationFailures, 'built_success' => $this->jwsBuiltSuccesses, 'built_failure' => $this->jwsBuiltFailures];
    }
}