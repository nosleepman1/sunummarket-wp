...
use Pymt_Vas\Dependencies\Jose\Component\KeyManagement\Analyzer\MessageBag;
...
class KeyCollector implements Collector
{
    ...
    public function __construct(private readonly ?KeyAnalyzerManager $jwkAnalyzerManager = null, private readonly ?KeysetAnalyzerManager $jwksetAnalyzerManager = null)
    {
    }
    /**
    ...
    private function collectJWK(array &$data): void
    {
        ...
        foreach ($this->jwks as $id => $jwk) {
            $data['key']['jwk'][$id] = ['jwk' => $cloner->cloneVar($jwk), 'analyze' => $this->jwkAnalyzerManager === null ? [] : $this->jwkAnalyzerManager->analyze($jwk)];
        }
    }
    /**
     * @param array<string, array<string, mixed>> $data
    ...
    private function collectJWKSet(array &$data): void
    {
        ...
        foreach ($this->jwksets as $id => $jwkset) {
            ...
            $data['key']['jwkset'][$id] = ['jwkset' => $cloner->cloneVar($jwkset), 'analyze' => $analyze, 'analyze_jwkset' => $analyzeJWKSet];
        }
    }
}